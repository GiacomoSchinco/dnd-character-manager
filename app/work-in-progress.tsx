import Link from "next/link";
export default function WorkInProgress(){
    return (
        <main className="min-h-screen flex items-center justify-center">
            <div className="container mx-auto p-6">
                <div className="max-w-lg mx-auto text-center">
                    <div className="inline-flex items-center justify-center w-36 h-36 rounded-full bg-primary/10 text-primary mb-6">
                        <span className="text-5xl font-extrabold">🚧</span>
                    </div>

                    <h1 className="text-2xl md:text-3xl font-bold mb-2">Pagina in lavorazione</h1>
                    <p className="text-sm md:text-base text-muted mb-6">
                        Questa pagina è attualmente in fase di sviluppo. Torna presto per nuove funzionalità!
                    </p>

                    <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                        <Link href="/" className="btn btn-primary">
                            Torna alla Home
                        </Link>
                    </div>
                </div>
            </div>
        </main>
    )
}